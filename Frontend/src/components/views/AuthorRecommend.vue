				...
				<ul class="others">
					<li :key="recommend.id" v-for="recommend in recommendList">
						<a :href="'/' + recommend.urlType + '/' +recommend.linkId">
							<p class="title">{{recommend.author}}</p>
						</a>
						<p class="info">
							<iv-icon type="compose"></iv-icon> {{'写了' + recommend.word_count + 'k字' + ' '}}
							<iv-icon class="likes" type="heart"></iv-icon> {{recommend.like_number + 'k喜欢'}}
						</p>
					</li>
				</ul>
			</div>
